...
<template>
  <div class="prm-page">
    ...
    <div class="prm-table-card">
      <PageNote>注:依据规则自动校验【材料完整性 / 权限合理性(先确后授·权益类型·授权范围) / 合规性(第三方许可·敏感数据·跨域) / 授权协议要素(使用场景·目的·利益分配·安全保障 附录D §3.4.4)】，输出红/黄/绿报告，记录可查询导出。</PageNote>
      <el-table :data="rows" v-loading="loading" border stripe>
        <el-table-column type="index" label="序号" width="60" align="center" />
        <el-table-column prop="applyId" label="申请ID" width="190" show-overflow-tooltip />
        <el-table-column prop="riskLevel" label="风险等级" width="100" align="center">
          <template #default="{ row }"><span :class="'prm-c-' + ((tag(row.riskLevel)) || 'primary')">{{ row.riskLevel }}</span></template>
        </el-table-column>
    ...
    <el-dialog v-model="reportDlg" title="合规校验报告（四维）" width="640px" align-center>
      <el-alert v-if="report" :type="report.riskLevel==='红'?'error':(report.riskLevel==='黄'?'warning':'success')" :closable="false" style="margin-bottom:10px">
      ...
      <el-table :data="report?.items || []" border size="small">
        <el-table-column prop="dimension" label="维度" width="110">
          <template #default="{ row }"><span class="prm-c-primary">{{ row.dimension }}</span></template>
        </el-table-column>
        <el-table-column prop="item" label="校验项" width="180" />
        <el-table-column label="结果" width="80" align="center">
          <template #default="{ row }"><span :class="'prm-c-' + ((row.pass?'success':'danger') || 'primary')">{{ row.pass?'通过':'不符' }}</span></template>
        </el-table-column>
...
const { query: q, rows, total, loading, load, search: onSearch } = useTablePage(pageAuthCompliance, { riskLevel: '' })
