<template>
  <div class="prm-page">
    ...
    <div class="prm-table-card">
      ...
      <div class="prm-table-note">注:授权指引提供最新的授权政策/流程图/常见问答/申请步骤,支持在线阅读与下载;列表仅显示各标题最新版本,同标题再次保存/上传将版本号自增并保留历史(可"版本历史"查看与回设)。</div>
      ...
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item v-if="!form.guidanceId" label="上传文件">
          <el-upload :auto-upload="false" :limit="1" :on-change="onFilePick" :on-remove="()=>{ pickedFile=null }">
            <el-button>选择文件(政策PDF/流程图/FAQ)</el-button>
    ...
    <!-- 在线查看 -->
    <el-dialog v-model="viewDlg" title="授权指引详情" width="560px" align-center>
...
import {
  ...
  uploadAuthGuidanceFile, authGuidanceVersions, setAuthGuidanceLatest, authGuidanceDownloadUrl
} from '@/api/authorize'

const types = ['政策文件', '流程图', '常见问答', '申请步骤', '材料样例']
...
async function load() {
  loading.value = true
  try { const r = await pageAuthGuidance({ ...q, latestOnly: true }); rows.value = r.records || []; total.value = r.total || 0 }
...
async function onSave() {
  ...
  try {
    if (!form.guidanceId && pickedFile) {
      ...
      await uploadAuthGuidanceFile(fd)
      ElMessage.success('已上传并入库为新版本')
    } else {
      await saveAuthGuidance({ ...form })
...
async function onSetLatest(row) {
  await setAuthGuidanceLatest(row.guidanceId)
  ElMessage.success(`已将 ${row.version} 设为最新`)
  verRows.value = await authGuidanceVersions(verTitle.value)
  load()
}
function onDel(row) {
  ElMessageBox.confirm('确认删除该指引版本吗', '提示', { type: 'warning' })
    .then(async () => { await deleteAuthGuidance(row.guidanceId); ElMessage.success('已删除'); load() }).catch(() => {})
}
onMounted(load)
</script>
