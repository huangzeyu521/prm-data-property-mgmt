    ...
    <div class="wz-body">
      ...
      <el-card v-show="step === 0" shadow="never">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="130px" :disabled="!!applyId" style="max-width:680px">
          ...
          <el-form-item label="使用场景及目的"><el-input v-model="form.scenario" /></el-form-item>
      ...
      <el-card v-show="step === 1" shadow="never">
        <div class="prm-table-note" style="margin-bottom:10px">校验授权范围是否≤确权边界、先确后授、材料完整性等。通过后方可提交审核。</div>
        <el-button type="primary" :loading="checking" @click="runCheck" style="margin-bottom:12px">执行合规校验</el-button>
        <el-descriptions v-if="checkResult" :column="1" border>
          ...
          <el-descriptions-item label="风险级别">{{ checkResult.riskLevel || '—' }}</el-descriptions-item>
          <el-descriptions-item label="问题/说明">{{ checkResult.problemDesc || '无' }}</el-descriptions-item>
        </el-descriptions>
      </el-card>
...
import { saveAuthDraft, submitAuth, runAuthCompliance } from '@/api/authorize'
