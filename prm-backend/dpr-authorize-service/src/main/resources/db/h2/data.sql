...
('AA-001','SQ-0001','一事一议','AST-001','客户用电信息表','EC-PRA-0001','广州供电局','使用权','电力金融征信','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'已生效',TRUE,CURRENT_TIMESTAMP,0),
('AA-002','SQ-0002','一事一议','AST-002','台区负荷数据','EC-PRA-0002','深圳供电局','使用权','负荷预测','约定字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'合规审核中',FALSE,CURRENT_TIMESTAMP,0),
('AA-003','SQ-0003','一事一议','AST-003','设备资产台账','EC-PRA-0003','南网科研院','使用权','设备健康分析','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'经理审核中',FALSE,CURRENT_TIMESTAMP,0),
('AA-004','SQ-0004','批量','AST-006','充电桩运营数据','EC-PRA-0006','广东电网综能公司','经营权','对外经营','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'领导小组审批中',TRUE,CURRENT_TIMESTAMP,0),
('AA-005','SQ-0005','批量','AST-008','线损分析数据','EC-PRA-0008','贵州电网','使用权','线损治理','约定字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'主管审核中',FALSE,CURRENT_TIMESTAMP,0),
('AA-006','SQ-0006','一事一议','AST-004','停电事件记录','EC-PRA-0004','广西电网','使用权','停电分析','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'已驳回',FALSE,CURRENT_TIMESTAMP,0);

-- 一事一议·单位初审中(表2 步骤50 分管领导单位初审):供「申报单位·分管领导(unit)」角色审核台/待办有真实在办队列。
-- 缺此种子则 unit 登录审核台/待办空无一物,无法演示其唯一职责(其余审批角色均有在办种子)。
INSERT INTO IM_AUTH_APPLY (CEC_APPLY_ID,CEC_APPLY_NO,CEC_AUTH_MODE,CEC_ASSET_ID,CEC_ASSET_NAME,CEC_EQUITY_CARD_ID,CEC_GRANTEE_ORG,CEC_RIGHT_TYPE,CEC_SCENARIO,CEC_SCOPE,CEC_VALID_DATE,CEC_STATUS,CEC_NEED_CONFIDENTIALITY,CEC_BUSINESS_DOMAIN,CEC_CREATE_TIME,CEC_DEL_FLAG) VALUES
('AA-008','SQ-0008','一事一议','AST-002','台区负荷数据','EC-PRA-0002','云南电网','使用权','负荷预测建模','约定字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'单位初审中',FALSE,'营销域',CURRENT_TIMESTAMP,0);
...
('BL-001','PLQD-2026-001','2026','批准',1,'2026年度批量授权清单',CURRENT_TIMESTAMP,0),
('BL-002','PLQD-2026-002','2026','申报稿',1,'综能板块批量授权',CURRENT_TIMESTAMP,0),
('BL-003','PLQD-2026-003','2026','草案',0,'科研院数据使用批量授权',CURRENT_TIMESTAMP,0),
('BL-004','PLQD-2026-004','2026','申报稿',2,'综能充电桩对外经营批量授权(明细已过副总,待领导小组终批)',CURRENT_TIMESTAMP,0);

-- 批量授权:补一条「领导小组审批中」批量明细(AA-007),与 AA-004 同挂 BL-004,演示「批准→协议自动生成」
INSERT INTO IM_AUTH_APPLY (CEC_APPLY_ID,CEC_APPLY_NO,CEC_AUTH_MODE,CEC_ASSET_ID,CEC_ASSET_NAME,CEC_EQUITY_CARD_ID,CEC_GRANTEE_ORG,CEC_RIGHT_TYPE,CEC_SCENARIO,CEC_SCOPE,CEC_VALID_DATE,CEC_STATUS,CEC_NEED_CONFIDENTIALITY,CEC_BATCH_LIST_ID,CEC_CREATE_TIME,CEC_DEL_FLAG) VALUES
('AA-007','SQ-0007','批量','AST-009','储能调度数据','EC-PRA-0009','广东电网综能公司','经营权','对外经营','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'领导小组审批中',TRUE,'BL-004',CURRENT_TIMESTAMP,0);

-- 修正历史种子:明细与清单的归属(原 ITEM_COUNT 仅为占位,未挂任何明细 → 批准/生成协议会落空)
UPDATE IM_AUTH_APPLY SET CEC_BATCH_LIST_ID='BL-004' WHERE CEC_APPLY_ID='AA-004'; -- 领导小组审批中
UPDATE IM_AUTH_APPLY SET CEC_BATCH_LIST_ID='BL-002' WHERE CEC_APPLY_ID='AA-005'; -- 主管审核中 → BL-002 终批应被链 guard 拦
UPDATE IM_AUTH_APPLY SET CEC_BATCH_LIST_ID='BL-001' WHERE CEC_APPLY_ID='AA-001'; -- 已生效 → BL-001(已批准)可去协议双签
...
-- 补所属系统名(CEC_SYSTEM_NAME,原种子未填,与向导选表落库对齐):按引用的权益卡片(IM_EQUITY_CARD_INFO)真实归属系统回填,
-- 供审核台/历史/批量清单「所属系统」列正确显示(assetId 是库表/卡片级 ID,不含 SYS: 前缀,不能直接当系统名解析)。
UPDATE IM_AUTH_APPLY SET CEC_SYSTEM_NAME='营销管理系统' WHERE CEC_APPLY_ID IN ('AA-001','AA-002','AA-003','AA-005','AA-007','AA-008');
UPDATE IM_AUTH_APPLY SET CEC_SYSTEM_NAME='计量自动化系统' WHERE CEC_APPLY_ID='AA-006';
UPDATE IM_AUTH_APPLY SET CEC_SYSTEM_NAME='生产管理系统' WHERE CEC_APPLY_ID='AA-004';
...
-- 授权运营协议(3,草案/要素落定/双签归档不同阶段)
INSERT INTO IM_AUTH_AGREEMENT (CEC_AGREEMENT_ID,CEC_AGREEMENT_NO,CEC_APPLY_ID,CEC_TEMPLATE_ID,CEC_GRANTEE_ORG,CEC_FILE_URL,CEC_SEAL_STATUS,CEC_GRANTOR_SIGNED,CEC_GRANTEE_SIGNED,CEC_REVIEW_STATUS,CEC_ARCHIVE_STATUS,CEC_DOC_STATUS,CEC_VALID_UNTIL,CEC_GEO_SCOPE,CEC_SECURITY_ENCRYPT,CEC_SECURITY_ACCESS,CEC_SECURITY_AUDIT,CEC_PENALTY_AMOUNT,CEC_DISPUTE_METHOD,CEC_SERVICE_DELIVERY,CEC_COPIES_COUNT,CEC_CONFIDENTIALITY_FILE,CEC_CREATE_TIME,CEC_DEL_FLAG) VALUES
('AG-001','XY-0001','AA-001','IT-007','广州供电局','/files/ag001.ofd','已双签',TRUE,TRUE,'通过','已归档','正式稿',DATEADD('YEAR',3,CURRENT_TIMESTAMP),'广东省行政区域内','加密传输(传输通道加密,存储加密)','最小授权访问控制,按需授予','全流程操作留痕、可追溯审计','10','向甲方所在地人民法院起诉','手机:020-88880001;邮箱:gzgdj@csg.cn',4,'/files/ag001-nda.pdf',CURRENT_TIMESTAMP,0),
('AG-002','XY-0004','AA-004','IT-007','广东电网综能公司','/files/ag004.ofd','待对方签章',TRUE,FALSE,'待审核','未归档','正式稿',DATEADD('YEAR',3,CURRENT_TIMESTAMP),'广东省行政区域内','加密传输(传输通道加密,存储加密)','最小授权访问控制,按需授予','全流程操作留痕、可追溯审计','20','向甲方所在地人民法院起诉','邮箱:zn@csg.cn',4,NULL,CURRENT_TIMESTAMP,0),
('AG-003','XY-0003','AA-003','IT-007','南网科研院','/files/ag003.ofd','待双方签章',FALSE,FALSE,'待审核','未归档','草案',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,CURRENT_TIMESTAMP,0);
...
('BA-001','BA-20260601001','AG-001','AA-001','广东电网','广州供电局','使用权','已备案',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,0),
('BA-002','BA-20260601002','AG-002','AA-004','广东电网综能公司','广东电网综能公司','经营权','待备案',NULL,CURRENT_TIMESTAMP,0),
('BA-003','BA-20260601003',NULL,'AA-005','贵州电网','贵州电网','使用权','待备案',NULL,CURRENT_TIMESTAMP,0);
...
('ACERT-001','AC-PRA-0001','AA-001','AST-001','广州供电局','使用权','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'生效',NULL,CURRENT_TIMESTAMP,0),
('ACERT-002','AC-PRA-0004','AA-004','AST-006','广东电网综能公司','经营权','全字段',DATEADD('DAY',20,CURRENT_TIMESTAMP),'生效',NULL,CURRENT_TIMESTAMP,0),
('ACERT-003','AC-PRA-0008','AA-005','AST-008','贵州电网','使用权','约定字段',DATEADD('YEAR',1,CURRENT_TIMESTAMP),'已暂停','监测联动熔断:越权调用',CURRENT_TIMESTAMP,0),
('ACERT-004','AC-PRA-0003','AA-003','AST-003','南网科研院','使用权','全字段',DATEADD('YEAR',2,CURRENT_TIMESTAMP),'生效',NULL,CURRENT_TIMESTAMP,0);
...
('ATPL-001','专项授权-使用权证书模板','v1','专项授权证书','使用权','兹证明被授权方 {被授权方} 经专项授权,对 {所属系统} / {数据表}(模式 {模式名称})享有 {权益类型}。授权范围:{授权范围};使用场景及目的:{使用场景及目的};授权期限至 {授权期限}。本证书依审核通过的授权协议与确权信息标准化生成,授权范围不超确权边界。证书编号:{证书编号}。','生效中',CURRENT_TIMESTAMP,0),
('ATPL-002','专项授权-经营权证书模板','v1','专项授权证书','经营权','兹证明被授权方 {被授权方} 经专项授权,对 {所属系统} / {数据表}(模式 {模式名称})享有 {权益类型}。授权范围:{授权范围};使用场景及目的:{使用场景及目的};授权期限至 {授权期限}。经营权对外提供数据产品/服务须在公司备案(附录G),授权范围仅限对外开放目录。证书编号:{证书编号}。','生效中',CURRENT_TIMESTAMP,0),
('ATPL-003','批量授权-使用权证书模板','v1','批量授权证书','使用权','兹证明被授权方 {被授权方} 经批量授权,对 {所属系统} / {数据表} 享有 {权益类型}(本批授权明细以《数据授权清单》为准)。使用场景及目的:{使用场景及目的};授权期限至 {授权期限}。证书编号:{证书编号}。','生效中',CURRENT_TIMESTAMP,0);
...
('AG-002','数据对外授权管理政策文件','政策文件','v1','公司总部',CURRENT_TIMESTAMP,'/files/对外授权政策.pdf',TRUE,'依"三权分置",取得经营权的单位对外提供数据须签协议并向数字化部备案。',CURRENT_TIMESTAMP,0),
...
('SC-001','内部经营分析','内部分析','使用权','本单位内部经营/运营数据统计与分析,不对外提供','用于本单位内部经营分析,数据不出域、不对外提供,使用范围限于授权部门。','生效中',CURRENT_TIMESTAMP,0),
('SC-002','对外数据服务','对外服务','经营权','向取得经营权的外部单位提供数据产品/服务','为对外提供数据产品/服务,被授权方仅在约定范围内使用,不得再授权或超范围使用。','生效中',CURRENT_TIMESTAMP,0),
('SC-003','联合建模','联合建模','使用权','与合作方在隐私计算/可信环境下联合建模','用于与合作方在隐私计算环境联合建模,原始数据不出域,仅交换模型/结果。','生效中',CURRENT_TIMESTAMP,0),
('SC-004','监管报送','监管报送','通用','按行政监管要求向监管机构报送数据','应行政监管要求报送数据,使用范围限于监管报送目的,符合相关法规。','生效中',CURRENT_TIMESTAMP,0),
('SC-005','科研合作','对外服务','经营权','向高校/科研机构提供脱敏数据用于科研','用于科研合作,提供脱敏数据,仅限约定科研课题使用,到期销毁。','停用',CURRENT_TIMESTAMP,0);
