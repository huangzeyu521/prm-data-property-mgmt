...
public class AitElementController {
    ...
    public Result<AitElementService.ProfileDTO> extract(@PathVariable String materialId,
                                                   @RequestParam(defaultValue = "true") boolean useModel) {
        return Result.success(service.extract(materialId, useModel));
    }

    /** 取材料的确权画像(主体/约束/结构化 JSON)。 */
    @GetMapping("/{materialId}/profile")
    public Result<AitElementService.ProfileDTO> profile(@PathVariable String materialId) {
        return Result.success(service.profile(materialId));
    }

    /** #6 表级 + 附件级 确权特征视图(按数据表/申请聚合)。 */
    @GetMapping("/view")
    public Result<Map<String, Object>> view(@RequestParam(required = false) String dataTableRef,
                                       @RequestParam(required = false) String applyId) {
        return Result.success(service.view(dataTableRef, applyId));
    }
}
