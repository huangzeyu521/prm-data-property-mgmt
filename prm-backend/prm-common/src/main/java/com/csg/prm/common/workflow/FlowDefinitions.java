...
public final class FlowDefinitions {
    ...
    static {
        ...
        // 一事一议(35号文 附录C表2 步骤20-100):单位初审(表2 20-50申报单位内部四级合并为一节点) -> 合规 -> 业务 -> 主管 -> 经理 -> 副总 -> 批准。
        // 终态=批准(待双签):协议双签+承诺函归档后由业务侧置「已生效」(表2 110-130 先签约后执行授权,生效不在本链)。
        FLOWS.put(DPR_AUTH_SPECIAL, List.of("单位初审中", "合规审核中", "业务审核中", "主管审核中", "经理审核中", "副总审批中", "批准"));
