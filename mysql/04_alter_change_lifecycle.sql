...
CREATE TABLE IF NOT EXISTS IM_DPR_AI_RUNLOG (
  ...
  CEC_BIZ_TYPE      VARCHAR(20)  COMMENT '业务域(确权/授权)',
  CEC_APPLY_ID      VARCHAR(64)  NOT NULL COMMENT '业务主键(申请ID)',
  CEC_CAPABILITY    VARCHAR(40)  COMMENT '能力类型(确权/授权 各校验能力)',
...
-- 6) 授权侧打通:授权申请单增 AI 校验快照列(防篡改包,供人工审核复核·可审计)
ALTER TABLE IM_AUTH_APPLY
  ADD COLUMN CEC_AI_SNAPSHOT LONGTEXT COMMENT '提交前AI校验结果快照(防篡改包),供人工审核复核·可审计' AFTER CEC_CONTACT;
